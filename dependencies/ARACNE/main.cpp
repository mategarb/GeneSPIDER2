...
* $Id: main.cpp,v 1.7 2011/03/03 16:25:12 manju Exp $ */
...
void usage( const char * prog )
{
  ...
  if(!infile.good()) {
    throw(string("Cannot find file \"usage.txt\"! Please make sure it is in the current working directory."));
  }
...
void findKernelWidth(int n, Parameter & p)
{
  ...
  if(!infile.good()) {
    throw(string("Cannot find file: "+filename+"! Please use '-H' option to specify the ARACNE home directory."));
  }
  ...
  if ( sin.good() && (sin.peek() != EOF) ) {
    ...
    throw(string("Configuration file format error: "+filename));
  }
  infile.close();
...
int run( int argc, char * argv[] )
{
   ...
   */
    ...
    int TYPE;
    if (cmpIgnoreCase( p.algorithm.c_str(), "fixed_bandwidth" )==0) {
      TYPE = Mutual_Info::MI_GAUSSIAN;
    }
    else if (cmpIgnoreCase( p.algorithm.c_str(), "variable_bandwidth" )==0) {
      TYPE = Mutual_Info::MI_GAUSSIAN_NO_COPULA;
    }
    else  {
      TYPE = Mutual_Info::MI_ADAPTIVE_PARTITIONING;
    }
    ...
    Mutual_Info mi(nsample , p.sigma, TYPE );
    ...
    if (nboot > 0)
    {
      ...
      if (p.outfile == "") {
          throw(std::string("An output file must be specified!"));
      }
  ...
  else
  {
    ...
    // Added entry to keep track of TFs for hub creation as well as DPI protection
    vector < int > transcriptionFactors;
    map < uint, uint > transfac;
    vector < int > low;
    vector < int > high;
    vector < int > bs;
    vector < int > * v = NULL;
    ...
    if (p.controlId != "") {
      controlId = data.getProbeId( p.controlId );
      if (controlId == -1) {
        throw(std::string("Cannot find marker: "+p.controlId+"!"));
      }
      bool isHigh = (p.condition == "+") ? true : false;
      data.getHighLowPercent( p.percent, controlId, low, high );
      v = ( isHigh ) ? & high : & low;
    ...
    // the data to pre-compute the variance of each gene's expression
    ...
    //Compute marker bandwidth
    data.computeMarkerBandwidth( v );
    ...
    if (p.hub != "") {
      ...
      if (hubId == -1) {
        throw(string("Cannot find the hub probe: "+p.hub+", nothing to be computed!"));
      }
      ids.push_back(hubId);
    }

    if (not(p.subnet.empty())) {
      for (uint i=0; i<p.subnet.size(); i++) {
    ...
    {
      for (uint i=0; i<p.tf_list.size(); i++)
      {
        ...
        if (gid == -1) {
          ...
          transfac[gid] = 1;
	  transcriptionFactors.push_back(gid);
        }
      }
    }
    ...
    if (p.adjfile != "") {
      ...
      if (cmpIgnoreCase(p.algorithm.c_str(),"fixed_bandwidth")==1 || cmpIgnoreCase(p.algorithm.c_str(),"adaptive_partitioning")==1){
