 ...
 * param.cc --
 *
 * $Id: param.cpp,v 1.3 2011/02/28 01:40:58 manju Exp $
 */
...
void checkParameter(Parameter &p) {
  if (p.infile == "") {
      throw(string("No input file specified!"));
  }

  if ( (p.subnetfile != "") && (p.hub != "") )
  {
    throw(string("Either supply one hub gene by '-h' or multiple genes in a file by '-l', but not both!"));
  }

  if (not((p.condition == "+") || (p.condition == "-") || (p.condition == ""))) {
    throw(string("Condition can be either '+' or '-', but no else!"));
  }

  if ((p.condition == "+") || (p.condition == "-")) {
    if (p.controlId == "") {
      throw(string("Control gene ID must be specified using '-c'!"));
    }
  }

  if ( (p.sigma != 99) && (p.sigma <= 0 || p.sigma >= 1) ) {
    throw(string("Kernel width '-k' must be within (0,1)!"));
  }

  if (p.threshold < 0) {
    throw(string("MI threshold '-t' must be non-negative!"));
  }
  ...
  if ( p.pvalue <= 0 || p.pvalue >1 ) {
    throw(string("P-value '-p' must be in the range (0,1]!"));
  }

  if (p.eps < 0 || p.eps>1) {
    throw(string("DPI tolerance '-e' must be within [0,1]!"));
  }

  if (p.percent <= 0 || p.percent >= 1) {
    throw(string("Percentage microarray must be within (0,1)!"));
  }

  if (p.mean < 0) {
    throw(string("Gene filter mean must be non-negative!"));
  }

  if (p.cv < 0) {
    throw(string("Gene filter cv (coefficient of variance) must be non-negative!"));
  }

  if (p.correction < 0) {
    throw(string("Array measurement noise level '-n' must be non-negative!"));
  }

  if (not(cmpIgnoreCase(p.verbose.c_str(),"on")==0 || cmpIgnoreCase(p.verbose.c_str(), "off")==0)) {
    throw(string("Verbose '-v' can be either 'on' or 'off', but no else!"));
  }

  if (not(cmpIgnoreCase(p.algorithm.c_str(),"variable_bandwidth")==0 || cmpIgnoreCase(p.algorithm.c_str(),"fixed_bandwidth")==0 || cmpIgnoreCase(p.algorithm.c_str(),"adaptive_partitioning")==0)) {
    throw(string("Supported algorithm '-a' is either 'variable_bandwidth', 'fixed_bandwidth' or 'adaptive_partitioning', and nothing else!"));
  }
...
void displayParameter(Parameter &p) {
  ...
  if (cmpIgnoreCase(p.algorithm.c_str(),"fixed_bandwidth")==0 || cmpIgnoreCase(p.algorithm.c_str(),"variable_bandwidth")==0)
  {
...
int readProbeList(string infile, vector<string> & probe_list)
{
  ...
  if(!in.good()) {
    throw(string("Problem with reading the file: "+infile));
  }
