...
nlinksBo = zeros(length(zetavec),straps);
...
for j = 1:straps
    ...
    if ~rawZeta
        ...
        zmin = 0;
        % find zero network
        %estA = Methods.lasso(bdata,net,zmax,true);
        while nnz(estA) > 0
            %tmp = zmax;
            %zmax = zmax*2;
            %estA = Methods.lasso(bdata,net,zmax,true);
        ...
        while zmax-zmin > tol
            i = (zmax + zmin) * 0.5;
            %estA = Methods.lasso(bdata,net,i,true);
            if nnz(estA) == 0
                zmax = i;
            else
                %zmin = i;
    ...
    for i = 1:size(bdata.P,1)
        %[~,fitinfo] = lasso(response(bdata,net)', -bdata.P(i,:)','CV',size(response(bdata,net),1),'Lambda',logspace(-5,0,50),'Alpha',1);
        %indMin = find(fitinfo.MSE == min(fitinfo.MSE));
        b = lasso(response(bdata,net)', -bdata.P(i,:)', 'Lambda', zetavec,'Alpha',1); %fitinfo.Lambda(indMin(1))
        Afit(i,:,:) = b(:,:);
    end
    %for i = 1:size(bdata.P,1)
