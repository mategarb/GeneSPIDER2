function varargout = lassolog(varargin)
...
for i = 1:size(data.P,1)
   [mdl,fitinfo] = fitclinear(data.Y', -data.P(i,:)','Learner','logistic','Prior','uniform','Regularization','lasso','Lambda',zetavec, Solver='sparsa');
    Afit(i,:,:) = mdl.Beta(:,:);
end
