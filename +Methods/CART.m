...
Afit = zeros(size(data.P,1),size(data.P,1));
    for i = 1:size(data.P,1)
        mdl = fitctree(data.Y', data.P(i,:),PredictorSelection="allsplits", Surrogate="on", NumBins=2);%,PredictorSelection="interaction-curvature",Surrogate="on", );
